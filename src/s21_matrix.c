...
int s21_create_matrix(int rows, int columns, matrix_t *result) {
  ...
  fi(rows) fj(columns) result->matrix[i][j] = 0;
  return OK;
}
  ...
  if (EQ(A, B)) {
    for (int i = 0; i < A->rows; ++i) {
      for (int j = 0; j < A->columns; ++j) {
        if (fabsl(A->matrix[i][j] - B->matrix[i][j]) > S21_EPS) return FAILURE;
      }
    }
  } else {
    return FAILURE;
...
int s21_sum_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  if (EQ(A, B)) {
    ...
    fi(A->rows) fj(A->columns) result->matrix[i][j] =
...
int s21_mult_number(matrix_t *A, double number, matrix_t *result) {
  int ans;
  ans = s21_create_matrix(A->rows, A->columns, result);
  fi(A->rows) fj(A->columns) result->matrix[i][j] = A->matrix[i][j] * number;
  return ans;
}
...
int s21_mult_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  if (A->columns == B->rows) {
    ...
    fi(A->rows) fj(B->columns) fk(A->columns) result->matrix[i][j] +=
...
int s21_transpose(matrix_t *A, matrix_t *result) {
  ...
  fi(A->rows) fj(A->columns) result->matrix[j][i] = A->matrix[i][j];
  return ans;
}
...
void print_matr(matrix_t *A) {
    ...
    fj(A->columns) printf("%f ", A->matrix[i][j]);
    printf("\n");
  }
  printf("\n");
}
...
int s21_calc_complements(matrix_t *A, matrix_t *result) {
  ...
  if (A->columns == A->rows) {
    ...
    if (A->rows >= 3) {
      fi(A->rows) fj(A->columns) {
        matrix_t minor;
        select_minor_matrix(A, &minor, i, j);
        result->matrix[i][j] = pow(-1, i + j) * just_determinant(&minor);
        s21_remove_matrix(&minor);
      }
    }
  } else {
